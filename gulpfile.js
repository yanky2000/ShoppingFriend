...
gulp.task('stylesConcat', function () {
    return combiner(
        gulp.src(cssDevPath),
        // cached('stylesConcat'), // instead of adding {since: gulp.lastRun('stylesConcat')} to gulp.src, to prevent ctrl-z bug of deleted files.
        remember('stylesConcat'),
        sourcemaps.init(),
        sass({ indentedSyntax: true }),
        ...
        //     }
        // }))
        concat('assets/css/main-styles.css'),
        // autoprefixer(),
        sourcemaps.write(),
        debug({ title: 'assets' }),
        gulp.dest('dist')
...
gulp.task('assets', function () {
    ...
    // return gulp.src('dev/**/*.{html,css,jpg,png,jpeg}', { since: gulp.lastRun('assets') }) // starts filtering after assets run for 2nd time
...
gulp.task('build', gulp.series(
    'clean',
    gulp.parallel('assets', 'js', 'jade', 'stylesConcat')
    // gulp.parallel('jade', 'styles')
));
